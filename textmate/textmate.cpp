...
#include <time.h>
#define SKIP_PARSE_THRESHOLD 1000
...
inline textstyle_t construct_style(std::vector<span_info_t> &spans,
                                   int16_t index) {
  textstyle_t res = {
...
inline bool textstyles_equal(textstyle_t &first, textstyle_t &second) {
  return first.italic == second.italic && first.bold == second.bold &&
         ...
         first.a == second.a && first.bg_r == second.bg_r &&
         first.bg_g == second.bg_g && first.bg_b == second.bg_b &&
         first.bg_a == second.bg_a && first.caret == second.caret &&
...
language_info_ptr Textmate::language() {
  return languages[current_language_id];
}
...
std::vector<list_item_t> Textmate::theme_extensions() {
  ...
  for (auto ext : extensions) {
    ...
    for (int i = 0; i < themes.size(); i++) {
      list_item_t item = {.name = ext.package["id"].asString(),
                          .description = ext.package["description"].asString(),
                          .value =
                              ext.path + "/" + themes[i]["path"].asString()};
    ...
    for (int i = 0; i < langs.size(); i++) {
      ...
      if (lang.isMember("extensions")) {
        ...
        for (int j = 0; j < exts.size(); j++) {
          ...
          list_item_t item = {.name = lang["id"].asString(),
                              .description =
                                  ext.package["description"].asString(),
                              .value = ex.asString()};
          res.push_back(item);
          break;
        }
...
theme_info_t Textmate::theme_info() {
  ...
  info.struct_a =
      color_info_t::nearest_color_index(strct.red, strct.green, strct.blue);
  info.ctrl_r = ctrl.red;
  info.ctrl_g = ctrl.green;
  info.ctrl_b = ctrl.blue;
  info.ctrl_a =
      color_info_t::nearest_color_index(ctrl.red, ctrl.green, ctrl.blue);
...
block_data_t *Textmate::previous_block_data() { return &_previous_block_data; }
...
void block_data_t::make_dirty() { dirty = true; }
...
block_data_ptr doc_data_t::next_block(int line) { return block_at(line + 1); }
