...
bool context_t::parse_scope(scope::types::scope_t &res) {
  ...
  do {
    if (it == last || (!isalnum(*it) && *it != '*' && *it < 0x80))
      break;
    while (it != last && (isalnum(*it) || *it == '_' || *it == '-' ||
                          *it == '+' || *it == '*' || *it > 0x7F))
      ++it;
  } while (parse_char("."));
...
bool context_t::parse_group(any_ptr &res) {
  ...
  char const *bt = it;
  group_t group;
  if (parse_char("(") && parse_selector(group.selector) && ws() &&
      parse_char(")")) {
...
bool context_t::parse_filter(any_ptr &res) {
  ...
  filter_t filter;
  char side;
  if (parse_char("LRB", &side) && parse_char(":") && ws() &&
      (parse_group(filter.selector) || parse_path(filter.selector))) {
...
bool context_t::parse_expression(expression_t &res) {
  ...
  if (parse_char("-") && ws())
    res.negate = true;
  return parse_filter(res.selector) || parse_group(res.selector) ||
         parse_path(res.selector);
}
