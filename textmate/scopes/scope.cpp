...
scope_t::node_t::node_t(std::string const &atoms, node_t *parent)
    : _atoms(atoms), _parent(parent), _retain_count(1),
...
void scope_t::node_t::retain() { ++_retain_count; }
...
char const *scope_t::node_t::c_str() const { return _atoms.c_str(); }
...
scope_t::scope_t(scope_t &&rhs) { std::swap(node, rhs.node); }
...
bool scope_t::empty() const { return !node; }
...
std::string xml_difference(scope_t const &from, scope_t const &to,
  ...
  while (fromIter != fromScopes.rend() && toIter != toScopes.rend() &&
         *fromIter == *toIter)
    ++fromIter, ++toIter;
...
std::string to_s(scope_t const &s) { return (std::string)s; }

std::string to_s(context_t const &c) {
  return c.left == c.right ? to_s(c.left)
                           : to_s(c.left) + "\037" + to_s(c.right);
}
...
bool selector_t::does_match(context_t const &scope, double *rank) const {
  if (selector)
    return scope.left == wildcard || scope.right == wildcard ||
           selector->does_match(scope.left, scope.right, rank);
  if (rank)
    *rank = 0;
  return true;
}
