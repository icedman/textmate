...
static void get_settings_color(Json::Value const &item, color_info_t *target) {
  ...
  target->alpha = rgba[3];
  target->index =
      nearest_color(target->red * 255, target->green * 255, target->blue * 255);

  if (current_parsed_theme) {
    if (current_parsed_theme->colorIndices.find(target->index) ==
        current_parsed_theme->colorIndices.end()) {
      color_info_t clr;
      clr.red = target->red;
...
style_t theme_t::shared_styles_t::parse_styles(Json::Value const &item,
  ...
  if (settings.isMember("fontStyle")) {
    ...
    bool hasStrikethrough =
        fontStyle.find("strikethrough") != std::string::npos;

    res.bold = hasBold ? bool_true : (hasPlain ? bool_false : bool_unset);
    res.italic = hasItalic ? bool_true : (hasPlain ? bool_false : bool_unset);
    res.underlined =
        hasUnderline ? bool_true : (hasPlain ? bool_false : bool_unset);
    res.strikethrough =
        hasStrikethrough ? bool_true : (hasPlain ? bool_false : bool_unset);
  }
...
theme_t::theme_t(Json::Value const &themeItem, std::string const &fontName,
                 float fontSize)
    : _font_name(fontName), _font_size(fontSize) {
...
color_info_t theme_t::foreground() const { return _styles->_foreground; }
...
theme_t::find_shared_styles(Json::Value const &themeItem) {
  ...
  // if (theme == Cache.end())
  //     theme = Cache.emplace(uuid,
  //     std::make_shared<shared_styles_t>(themeItem)).first;
  // return theme->second;
}
...
style_t const &theme_t::styles_for_scope(scope::scope_t const &scope) {
  ...
  // static style_t _res;
  style_t _res =
      style_t(base.font_name, base.font_size, base.foreground, base.background,
              base.caret, base.selection, base.bold, base.italic,
              base.underlined, base.strikethrough, base.misspelled);
...
style_t &style_t::operator+=(style_t const &rhs) {
  ...
  strikethrough =
      rhs.strikethrough == bool_unset ? strikethrough : rhs.strikethrough;
