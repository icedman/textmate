  ...
  if (start) {
    ...
    add_token:
      if (end - start > 0) {
        ...
        argv[*argc] = token;
        if (++*argc + 1 == alloc) {
          argv = (char **)realloc(argv, (alloc *= 2) * sizeof(char *));
        }
      }
      in_token = false;
      escaped = false;
...
static inline char *argsep_next_interesting(const char *src,
                                            const char *delim) {
  char *special = (char *)strpbrk(src, "\"'\\");
  char *next_delim = (char *)strpbrk(src, delim);
  if (!special) {
    return next_delim;
  }
  if (!next_delim) {
