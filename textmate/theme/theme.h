...
struct color_info_t {
  color_info_t() : red(-1), green(0), blue(0), alpha(1), index(0) {}
  color_info_t(double red, double green, double blue, double alpha = 1)
      : red(red), green(green), blue(blue), alpha(alpha), index(0) {}
...
struct style_t {
  ...
  style_t(std::string const &fontName, float fontSize, color_info_t foreground,
          color_info_t background, color_info_t caret, color_info_t selection,
          bool_t bold, bool_t italic, bool_t underlined, bool_t strikethrough,
          bool_t misspelled)
      : font_name(fontName), font_size(fontSize), foreground(foreground),
        background(background), caret(caret), selection(selection), bold(bold),
  ...
  style_t(scope::selector_t const &scopeSelector = scope::selector_t(),
      ...
      : scope_selector(scopeSelector), font_name(fontName), font_size(fontSize),
        bold(bool_unset), italic(bool_unset), underlined(bool_unset),
        strikethrough(bool_unset), misspelled(bool_unset) {}
