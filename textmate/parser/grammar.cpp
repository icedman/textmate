...
void set_extensions(extension_list *exts) { extensions = exts; }
...
static void compile_patterns(rule_t *rule) {
  ...
  if (rule->while_string != NULL_STR &&
      !pattern_has_back_reference(rule->while_string)) {
  ...
  if (rule->end_string != NULL_STR &&
      !pattern_has_back_reference(rule->end_string)) {
...
void *grammar_t::setup_includes_thread(void *arg) {
  grammar_t::setup_includes_payload_t *p =
      (grammar_t::setup_includes_payload_t *)arg;
  if (p->path != "") {
...
#ifndef DISABLE_ADD_GRAMMAR_THREADS
      pthread_t thread_id;
      pthread_create(&thread_id, NULL, &setup_includes_thread, (void *)(p));
#endif
    } else {
...
rule_ptr grammar_t::add_grammar(std::string const &scope,
                                std::string const &path, rule_ptr const &base,
                                bool spawn_thread) {
#ifdef DISABLE_ADD_GRAMMAR_THREADS
  spawn_thread = false;
#endif
...
rule_ptr rule_find_rule(rule_ptr rule, int rule_id) {
  ...
  if (rule->begin_captures) {
    std::map<std::string, rule_ptr>::iterator it =
        rule->begin_captures->begin();
