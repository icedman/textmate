...
struct rule_t {
  ...
  rule_t()
      : rule_id(++rule_id_counter), include_string(NULL_STR),
        scope_string(NULL_STR), content_scope_string(NULL_STR),
        match_string(NULL_STR), while_string(NULL_STR), end_string(NULL_STR),
        apply_end_last(NULL_STR) {}
...
struct stack_t {
  ...
  stack_t(rule_t *rule, scope::scope_t const &scope,
          stack_ptr const &parent = stack_ptr())
      : parent(parent), rule(rule), scope(scope), anchor(0) {}
  ...
  std::string content_scope_string =
      NULL_STR; // expanded version of rule->content_scope_string

  regexp::pattern_t while_pattern; // a while-pattern active in current
  regexp::pattern_t end_pattern;   // the end-pattern which exits
  // this context
  size_t anchor;
  bool zw_begin_match;
  bool apply_end_last;
